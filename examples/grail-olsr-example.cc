...
main (int argc, char *argv[])
{
  ...
  // LogComponentEnable("TapBridge", LOG_LEVEL_ERROR);
  // LogComponentEnable("SyscallWrapper", LOG_LEVEL_ERROR);
  ...
  // LogComponentEnable("SyscallWrapper", LOG_LEVEL_LOGIC);
  // LogComponentEnable("SyscallWrapperNetlink", LOG_LEVEL_LOGIC);
...
#define OLSR_HG  1
  ...
  YansWifiChannelHelper wifiChannel;// = YansWifiChannelHelper::Default (); // DON'T TOUCH THIS!
  ...
  // auto hnaDevs = pointToPoint.Install(hnaNodes.Get(0), hnaNodes.Get(1));
  ...
  if(olsrVariant == OLSR_HG) {
    ...
    for(size_t i=0; i<nodes.GetN(); ++i) {
      ...
      // std::function<void()> g = [rt]() {
      //   rt->PrintRoutingTable(Create<OutputStreamWrapper>(&std::cerr));
      // };
      // Simulator::Schedule(Seconds(120), MakeFunctionalEvent(g));

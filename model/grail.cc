...
struct GrailApplication::Priv
{
  ...
  // Stuff that could as well be attributes:
  uint32_t gid;
  uint32_t egid;
  uint32_t uid;
  uint32_t euid;  
  ...
  std::map<int,Ptr<Socket> > m_sockets;
  std::map<int,std::tuple<Ptr<Socket>,Address>> m_fakeAcceptedSockets; // fd is from LISTEN socket, not new FD!!
  std::set<int> m_tcpSockets;
  std::set<int> m_connectedSockets;
  std::set<int> m_isListenTcpSocket;
  ...
  void ProcessStatusCode(SyscallHandlerStatusCode res, int syscall, pid_t pid) {
    if(res == SYSC_MANUAL) {
      ...
      Simulator::Schedule(app->m_syscallProcessingTime + pollLoopDetector.GetDelay(), &Priv::HandleSyscallBefore, this, pid);
      return;
  ...
  void HandleSyscallBefore(pid_t pid) {
    ...
    case SYS_mprotect:
      ...
      // file system io only (for now ignored):
    case SYS_fstat:
    case SYS_unlink:
    case SYS_access:
    case SYS_mmap:
    ...
    case SYS_openat: // in.tftpd uses this file/directory access
      ...
      // ...works on sockets (dangerous), but likely harmless on normal file-FDs:
    case SYS_dup:
    case SYS_stat:
      ...
      // (partly or fully) re-implemented syscalls (FIO):
    case SYS_read:
      res = HandleRead();
      break;
    case SYS_open:
    ...
    case SYS_close:
      ...
      // (partly or fully) re-implemented syscalls:
    case SYS_pipe2:
      res = HandlePipe2();
      break;
    case SYS_getpid:
  ...
  bool SetBsdAddress(const Address& ns3Address, struct sockaddr* addr, socklen_t* addrlen)
  {
    ...
    // yes: copy minimum, but write struct size...
    //  - consult recvfrom(2) for gory details.
  ...
  void MakeBsdAddress(const ns3::Ipv4Address& ns3Address, struct sockaddr_in* addr)
  {
    ...
    //    addr->sin_port   = htons(ns3InetAddr.GetPort());
  ...
  */
  ...
  SyscallHandlerStatusCode HandleGetRUsage() {
    ...
    // idea for improvement: use a heuristic here that serves the purpose of causing as little confusion as possible to processes that depend on the provided information.
  ...
  SyscallHandlerStatusCode HandleSetSockOpt() {
    ...
    if(level == SOL_SOCKET) {
      ...
      if (optname == SO_BROADCAST) {
        ...
        // no equivalent in ns-3, but there are not generally lingering sockets in ns-3 either 
        FAKE(0);
        return SYSC_SUCCESS;
      } else if (optname == SO_RCVBUF) {
        int myoptval = 0;
  ...
  SyscallHandlerStatusCode HandleSelect()
{
    ...
    for(auto& kv : m_sockets) {
      ...
      if(readfds && FD_ISSET(kv.first,&myreadfds)) {
        ...
        if(kv.second->GetRxAvailable()) {
          ...
          std::function<void(Ptr<Socket>, const Address&)> fakeAccept = \
            [this,readfds,writefds,exceptfds,fdnr,already_handled,timeout_event,defaultFakeAccept, saved_pid] \
  ...
  SyscallHandlerStatusCode HandleRecvMsg()
  {
    ...
    //   NS_ASSERT(flags == 0 && "UNIMPLEMENTED");
  ...
  SyscallHandlerStatusCode HandleAlarm() {
    NS_ASSERT(false && "alarm call handling is reportedly broken as of now");
    ...
    Simulator::Schedule(Seconds(seconds), MakeFunctionalEvent(cb));
...
//   SyscallWrapper w;
//   return w.StartProcess(argc-1, argv+1);
// }
